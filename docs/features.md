...
| Staff appointment operations | `appointments/index.blade.php` | `WebAppointmentController`, `AppointmentService`, `ExpirePendingAppointments` | appointments; staff web routes; scheduler/queue | Pending requests expire when their requested time passes and stop appearing in actionable pending lists. State transitions, policies, notifications, attendance and Jitsi links intersect here. |
...
| Assignments | staff assignment list/create/review; portal/mobile assignment screens | `WebAssignmentController`, `PortalAssignmentController`, API controller, `AssignmentService`, `AssignmentBadgeService` | assignments, submissions, private files | Submission/worksheet download and preview are policy-gated. File storage must remain private. Sidebar badges show clinicians their unreviewed submissions and patients their submissions awaiting review. |
...
| Mood logs | portal/mobile mood pages | `PortalMoodLogController`, API `MoodLogController`, `MoodLogService`, `MoodLogDates` | `mood_logs`, `mood_log_archives` | One immutable check-in per patient per Asia/Manila date. Clients consume server-provided `logged_on`; duplicate submissions conflict rather than overwrite. |
...
| Clinic chatbot | portal/mobile chatbot screens; admin content CRUD | `ChatbotService`, `ChatbotIntent/Response`, API/portal controllers | chatbot tables; Gemini optional | Explicit crisis language is handled locally; strong clinic FAQ matches return approved text. Gemini may provide qualified, low-risk general information about therapy expectations and healthcare concepts without presenting it as TheraConnect policy. Clinic-specific facts still require retrieved evidence, and generated wellbeing suggestions stay within a low-risk toolbox and point back to a clinician. |
