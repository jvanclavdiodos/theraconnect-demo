...
| Profile/avatar/password | staff account, portal profile, Flutter profile screens | `AccountController`, `PortalProfileController`, `ProfileController`, `PasswordController` | users/patients; profile API | Patient clients crop avatars to a square before upload. The shared request requires an image no larger than 2 MB and 1024x1024; avatar access remains authorization-gated. |
